# Per-screen card: `/application/attestation`
...
| Field | Value |
| --- | --- |
| Route(s) | `/application/attestation` |
| Source file | `src/app/(provider)/application/attestation/page.tsx` |
| Dynamic | no |
| Group | provider |
| Roles allowed | PROVIDER (token-authenticated) |
| Roles denied (must redirect/403) | all staff roles (redirect to signin) |
...
_TODO: add per-screen specs as they're written (e.g. `tests/e2e/provider/application__attestation.spec.ts`)._
