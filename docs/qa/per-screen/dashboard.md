...
| --- | --- |
| Route(s) | `/dashboard` |
| Source file | `src/app/(staff)/dashboard/page.tsx` |
| Dynamic | no |
| Group | staff |
| Roles allowed | every authenticated staff role |
| Roles denied (must redirect/403) | PROVIDER (redirect to signin) |
| PHI fields rendered | Aggregate counts only -- NO row-level PHI on this surface. |
...
_TODO: add per-screen specs as they're written (e.g. `tests/e2e/staff/dashboard.spec.ts`)._
